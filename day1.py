...
def conv_word(input: str) -> str:
    return valid_word[valid_word.index(input) - 10]
...
for word in input:
    ...
    # find first word/num
    for vw in valid_word:
        vw_ix = word.find(vw)
        if vw_ix != -1 and vw_ix < l_ix:
            l_ix = vw_ix
            l_w = vw

    # should have first valid word/num in l_w
    if not l_w.isnumeric():
        l_w = conv_word(l_w)

    # reverse current word and reverse each entry in the valid word list, and do the same
    r_ix = 9999
    r_w = ""
    ...
    for vw in valid_word:
        revword = word[::-1]
        rev_vw = vw[::-1]
        vw_ix = revword.find(rev_vw)
        if vw_ix != -1 and vw_ix < r_ix:
            r_ix = vw_ix
            r_w = vw

    if not r_w.isnumeric():
        r_w = conv_word(r_w)

    if (not l_w) and (not r_w):
        output.append("")
    else:
        output.append(int("".join([str(l_w), str(r_w)])))

for i in range(len(input)):
    print(f"val {output[i]} for input {input[i]}")
print(sum(output))
