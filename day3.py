...
MAX_BOARD_X = len(input_data[0])
MAX_BOARD_Y = len(input_data)
...
# if so, mark that number as "valid" (and add to valid array)
# sum that mf array
...
# we've got an array of "numbers". check each number to see if symbols around
part_numbers: [int] = []
for test_number in map_numbers:
    is_part_number = False
    n_x, n_y = test_number.get_origin()
    for x_val in range(n_x, n_x + test_number.number_len()):
        # should be going from x vals from start to end of number
        if is_symbol_surrounding(x_val, n_y):
            is_part_number = True
            break
    if is_part_number:
