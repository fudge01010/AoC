...
# set up some structs to make the logic clearer later
class Play:
...
class Game:
    ...
    def add_play(self, r, g, b):
        self._plays.append(Play(int(r), int(g), int(b)))
...
for gamestr in inputs:
    # get the game ID first
    g_id = gamestr.split(":")[0].split(" ")[1]
    temp_g = Game(g_id)
    # split the games from the game ID, then split each "play" apart
    play_strings = gamestr.split(": ")[1].split("; ")
    # split each play of the game apart into it's colours (if there are more than 1)
    for gs in play_strings:
        colours = gs.split(", ")
        c_r, c_g, c_b = 0, 0, 0
        for c in colours:
            if "red" in c:
                ...
                c_b = c.split(" ")[0]
        # should have r,g,b count for that play now - append it to our temp game
        temp_g.add_play(c_r, c_g, c_b)

    # all processed for that game, add it to our game list and do the next one
    games.append(temp_g)

# we now have games[Game], each Game has games.plays[Play].
# now we can iterate over the list and find the ID of "valid" games
MAX_RED = 12
MAX_GREEN = 13
MAX_BLUE = 14
...
for test_game in games:
    valid = True
    for test_play in test_game.plays():
        if (
            (test_play.r > MAX_RED)
            or (test_play.g > MAX_GREEN)
            or (test_play.b > MAX_BLUE)
        ):
            # game is NOT valid
            valid = False

    # print(f"validity of game {test_game._id} is {valid}")
    if valid:
        valid_games.append(test_game.id())

print(f"Valid game ID sum is {sum(valid_games)}")


# ================ PART 2 =================================
min_cubes_powers: [int] = []

for test_game in games:
    max_r, max_b, max_g = 0, 0, 0

    for test_play in test_game.plays():
        max_r = max(test_play.r, max_r)
        max_g = max(test_play.g, max_g)
        max_b = max(test_play.b, max_b)

    # print(f"validity of game {test_game._id} is {valid}")
    game_cube_pow = max_r * max_b * max_g
    print(
        f"game {test_game._id}: {max_r} red, {max_g} green, {max_b} blue.\nCube power is {game_cube_pow}"
    )
    min_cubes_powers.append(game_cube_pow)

print(f"Sum of powers: {sum(min_cubes_powers)}")
